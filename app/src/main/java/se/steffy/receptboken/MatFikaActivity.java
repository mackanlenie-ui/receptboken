...
/** Offline recipe collection with a responsive native Android interface. */
public class MatFikaActivity extends BatteryOptimizedActivity {
    ...
    @Override void load() {
        // The fresh app has a separate application ID and private storage.
        String data=prefs.getString("data", "");
        if(data.isEmpty() || !prefs.getBoolean("matfika_seed_v1", false)) {
            try (InputStream in=getAssets().open("recipes.json")) {
                ...
                if(!data.isEmpty()) {JSONArray old=new JSONArray(data); for(int i=0;i<old.length();i++)recipes.add(Recipe.from(old.getJSONObject(i)));}
                for(int i=0;i<a.length();i++){Recipe r=Recipe.from(a.getJSONObject(i));if(findRecipe(r.name)==null)recipes.add(r);}
                save(); prefs.edit().putBoolean("matfika_seed_v1",true).apply(); return;
            } catch(Exception e){ Toast.makeText(this,"Receptsamlingen kunde inte läsas",Toast.LENGTH_LONG).show(); }
        }
        recipes.clear();
        try {JSONArray a=new JSONArray(data);for(int i=0;i<a.length();i++)recipes.add(Recipe.from(a.getJSONObject(i)));}
        catch(Exception e){Toast.makeText(this,"Sparade recept kunde inte läsas. Återställ en säkerhetskopia.",Toast.LENGTH_LONG).show();}
    }
    ...
    /** Small vector artwork, drawn locally without network requests. */
