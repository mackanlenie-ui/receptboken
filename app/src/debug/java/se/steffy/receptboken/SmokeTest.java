...
public class SmokeTest extends Instrumentation {
    ...
    @Override public void onStart(){Bundle results=new Bundle();try{
        ...
        shot("home");
        onMain(()->{a.group="Kakor & bakning";a.home();check(a.resultCount.getText().toString().contains("8"),"Baking filter");a.search.setText("kakao");check(a.resultCount.getText().toString().contains("3"),"Ingredient search");a.query="";a.group="Alla";a.detail(a.findRecipe("Kladdkaka"),8);});waitForIdleSync();shot("recipe");onMain(()->checkBounds(a.root));onMain(()->a.page.fullScroll(View.FOCUS_DOWN));shot("recipe-steps");
    ...
    void checkBounds(android.view.ViewGroup parent){
        for(int k=0;k<parent.getChildCount();k++){View v=parent.getChildAt(k);check(v.getLeft()>=parent.getPaddingLeft()-1,"Child clipped at left: "+v.getClass().getSimpleName());check(v.getRight()<=parent.getWidth()-parent.getPaddingRight()+1,"Child clipped at right: "+v.getClass().getSimpleName());if(v instanceof android.view.ViewGroup)checkBounds((android.view.ViewGroup)v);}
    }
    void shot(String name)throws Exception{waitForIdleSync();SystemClock.sleep(800);waitForIdleSync();Bitmap b=getUiAutomation().takeScreenshot();File dir=new File(getTargetContext().getExternalFilesDir(null),"screenshots");dir.mkdirs();try(FileOutputStream f=new FileOutputStream(new File(dir,name+".png"))){b.compress(Bitmap.CompressFormat.PNG,100,f);}b.recycle();}
}
